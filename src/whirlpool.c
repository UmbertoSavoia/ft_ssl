...
{
    bzero(&ctx, sizeof(t_whirlpool_ctx));
}

void    whirlpool_transform(uint64_t m[])
{
    uint64_t k[8] = {0};
    uint64_t l[8] = {0};
    uint64_t s[8] = {0};
...
void    whirlpool_final(uint8_t digest[])
{
    uint64_t *m = 0;
    uint32_t offset = ctx.size % 64;
    uint32_t pad_len = offset < 32 ? 32 - offset : (64 + 32 - offset);
